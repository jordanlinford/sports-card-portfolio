...
- **SSP/Case Hit Parallel Detection**: Premium unnumbered parallels (Zebra, Tiger Stripe, Color Blast, Shock, Downtown, Kaboom, Mojo, Shimmer, The Man, Warp Speed, Interstellar, Eye of the Tiger, Supernova, Magician, Phenomenon, Street Art, Art Deco, Aurora, Ascension, etc.) are explicitly detected and priced separately from base/silver parallels. When a card IS an SSP/Case Hit, the prompt tells Gemini to INCLUDE SSP-tagged listings as comps (instead of excluding them). Search broadening protects SSP keywords from being dropped. SSP cards are exempted from outlier protection and cross-validation guards since wide price ranges are expected. Optic SSP inserts have enhanced Gemini prompts to prevent confusion with base Donruss pricing.
