...
- **Social Features**: Liking, commenting (including guest comments without login), prestige system, and bookmarking. Display case share links include OG meta tags with dynamic images for rich previews on social media.
...
- **Supply Saturation Alert**: Yellow "Supply Alert" badge shown when Gemini detects surging PSA grading volume for a card. Both `fetchGeminiMarketData` and `fetchUnifiedCardAnalysis` return `supply.supplyGrowth` ("stable"/"growing"/"surging"), `supply.supplyNote`, and optional `supply.estimatedPopulation`. Badge appears on card outlook detail page, quick analysis results, and display case card views. Pro users see the detailed supply note; free users see badge only. Data stored in existing Gemini market cache and persisted to `cards.outlook_supply_growth` column for display case rendering.
...
- **SSP/Case Hit Parallel Detection**: Premium unnumbered parallels (Zebra, Tiger Stripe, Color Blast, Shock, Downtown, Uptown, Kaboom, Mojo, Shimmer, The Man, Warp Speed, Interstellar, Eye of the Tiger, Supernova, Magician, Phenomenon, Street Art, Art Deco, Aurora, Ascension, etc.) are explicitly detected and priced separately from base/silver parallels. Gemini prompts use a "USE YOUR HOBBY KNOWLEDGE" approach — instead of hardcoded include/exclude rules, Gemini determines whether the card's variation is a premium insert/case hit/SSP and includes or excludes SSP-tagged listings accordingly. The hardcoded SSP pattern list is still used app-side for outlier protection bypass and cross-validation guard skipping. Search broadening protects SSP keywords from being dropped. SSP cards are exempted from outlier protection and cross-validation guards since wide price ranges are expected. Optic SSP inserts have enhanced Gemini prompts to prevent confusion with base Donruss pricing.
- **Chrome Insert Scanner Accuracy**: Scanner prompt explicitly warns that many insert subsets (Deep Space, Warp Speed, Interstellar, Instant Impact, Fireworks, Emergent, etc.) are inherently chrome/reflective — do NOT assume "Silver Prizm" just because the card is shiny. Common inserts (Deep Space, Warp Speed, etc.) are classified as base inserts in search query building to avoid SSP pricing contamination.
...
- **Gemini-First Pricing Philosophy**: The unified Gemini analysis (with Google Search grounding) is the primary pricing source. Legacy price lookups and monthly trend data serve as fallbacks only when Gemini returns no price. Cross-validation guards are log-only for high comp counts (4+). For LOW comp counts (0-3 Gemini comps), cross-validation actively caps Gemini at 2x legacy median when Gemini exceeds 2.5x legacy — prevents wildly inflated estimates from wrong/mixed comps. Mid-tier parallel prompts enforce exact parallel name matching to reduce Gemini comp contamination.
