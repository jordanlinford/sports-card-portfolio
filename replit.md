...
**Card Outlook page graduation (Track B step 1)**: `client/src/pages/card-outlook.tsx` `ACTION_STYLES` table now uses the FaithfulNeon 6-color verdict palette (BUY=emerald, MONITOR=amber, WATCH=slate, SELL=violet, LONG_HOLD/LEGACY_HOLD=blue — collapsing the prior indigo LEGACY_HOLD into the HOLD family per spec). `SignalBar` ditched the generic shadcn `<Progress>` for a custom bar that uses the `signal-bar-tier-{strong|good|neutral|weak|poor}` glow primitives at the same tier thresholds as Player Outlook's `MarketSignalsPanel` (Card Outlook signals are 0-10 so values are converted to a 0-100 percentage before tier lookup; the helper functions `getSignalScoreColor100` / `getSignalBarTier100` mirror Player Outlook's `getSignalColor` / `getSignalBarColor`). `CompositeScoreCard` reuses `getSignalScoreColor100` so a 70+ score reads as "strong" identically across both pages. Two semantic flag swaps: Big Mover badge purple→pink (echoes wordmark gradient + restores distinctness on the new dark-purple ground) and Supply Alert yellow→amber (aligns with the FaithfulNeon warning tier). Confidence indicator (HIGH/MEDIUM/LOW) intentionally stays on generic green/yellow/red — semantic traffic-light, not a verdict.
