...
- **Alpha Feed UI**: Public `/alpha` page displaying buy/sell/hold signals from the Alpha Engine. Three tabs: Opportunities (buy signals), Risks (sell signals), Trending (most-scanned cards). Portfolio Alerts section (auth-required) shows signals for user's own cards. Signal cards display "High Conviction" / "Early Signal" / "Emerging Risk" / "Market Watch" labels with specific market drivers (up to 3) and "Why now?" timing context. Thumbs up/down feedback on each signal (stored in `signal_feedback` table). Interaction tracking (alpha_view, signal_click, signal_expand, card_revisit) via `POST /api/alpha/track`. APIs: `GET /api/alpha/feed` (public), `GET /api/alpha/portfolio-alerts` (auth-required), `POST /api/alpha/signals/:signalId/feedback` (auth-required), `POST /api/alpha/track`. Page: `client/src/pages/alpha-feed.tsx`. Nav: "Alpha" with "New" badge in desktop + mobile navigation.
