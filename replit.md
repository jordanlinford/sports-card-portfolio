...
- **Batch Scanning** (Pro-only): Multi-image upload batch scan on the Card Analysis page. Users select multiple card photos at once, which are processed sequentially with a progress bar. Each card shows status (pending/processing/done/failed) with thumbnails, identified details, and confidence badges. Max 20 cards per batch. Includes cancel button mid-batch and completion toast summary. All scans auto-save to scan_history. Non-Pro users see a crown icon and upgrade prompt. Object URLs are properly cleaned up on completion/cancel/unmount.
