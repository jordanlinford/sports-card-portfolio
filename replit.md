...
- **Batch Scanning**: "Batch Scan" mode on the Card Analysis page lets users scan multiple cards in sequence without navigating away. After each scan result, a "Scan Next Card" button appears. A running count and mini-list of scanned cards (thumbnails + names + values) is shown at the bottom. "Done" exits batch mode and navigates to Scan History. All scans auto-save to scan_history.
