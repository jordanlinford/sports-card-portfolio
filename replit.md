...
- **Alpha Feed UI**: Public `/alpha` page displaying buy/sell/hold signals from the Alpha Engine. Three tabs: Opportunities (buy signals), Risks (sell signals), Trending (most-scanned cards). Portfolio Alerts section (auth-required) shows signals for user's own cards. APIs: `GET /api/alpha/feed`, `GET /api/alpha/portfolio-alerts`. Page: `client/src/pages/alpha-feed.tsx`. Nav: "Alpha" with "New" badge in desktop + mobile navigation.
